# Changes

## Minor Features

1. Feed tab says both "Loading..." and "No messages yet. Say hello!" when the server's spinning up.
2. Allow the room owner to force `GAME_ABANDON` on behalf of a player after a determined timeout (e.g. 5 minutes).
3. Idle game area gets the DVD treatment, but with a playing card, and the card changed on every impact. That or just say "No Active Game".
4. Replace words with icons+tooltips wherever reasonable.
5. Color preferences should be 3 rows of 6.

## Major Features

1. Game Reporting
2. Admin view
   1. Flag(?) col on Users table
   2. Additional page in menu, control other users (mainly reset basic auth password)
3. Notifications - on friend request/accept, room invite...
