...
describe('CredentialRepository', () => {
  ...
  describe('findOrCreateUserByOidc', () => {
    it('should return existing user if found', async () => {
      ...
      const result = await repo.findOrCreateUserByOidc('oidc-123');

      expect(result).toBe(user);
      expect(db.insert).not.toHaveBeenCalled();
    });

    it('should create user with preferredUsername when available and not taken', async () => {
      db.limit
        .mockResolvedValueOnce([]) // findUserByCredential — no existing user
        .mockResolvedValueOnce([]); // username availability check — not taken
      const newUser = { id: 2, username: 'preferred', displayName: null };
      db.returning.mockResolvedValue([newUser]);

      const result = await repo.findOrCreateUserByOidc('oidc-456', 'preferred');

      expect(result).toBe(newUser);
      expect(db.insert).toHaveBeenCalled();
    });

    it('should create user with autogenerated username when no preferredUsername', async () => {
      db.limit
        .mockResolvedValueOnce([]) // findUserByCredential
        .mockResolvedValueOnce([]); // generateUniqueUsername availability check
      const newUser = { id: 3, username: 'user_abcd1234', displayName: null };
      db.returning.mockResolvedValue([newUser]);

      const result = await repo.findOrCreateUserByOidc('oidc-789');
