...
describe('ChatGateway', () => {
  ...
  beforeEach(() => {
    ...
    redisPubSub = {
      ...
      onChannel: jest.fn().mockReturnValue(of()),
    } as any;

    gateway = new ChatGateway(roomService, chatService, redisPubSub);
  });
  ...
  describe('handleChatSend', () => {
    ...
    it('should persist message and publish to Redis', async () => {
      const client = createMockClient();
      gateway.handleConnection(client);
      await gateway.handleJoinRoom(client, { roomId: 'r1', nickname: 'Alice' });
      ...
      await gateway.handleChatSend(client, { roomId: 'r1', content: 'Hello' });

      expect(chatService.saveMessage).toHaveBeenCalled();
      expect(redisPubSub.publish).toHaveBeenCalledWith('chat:messages', mockMsg);
    });
  });
